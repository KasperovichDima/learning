...
The algorithm for myAtoi(string s) is as follows:
    ...
    [-231, 231 - 1], then clamp the integer so that it remains in
    the range. Specifically, integers less than -231 should be clamped
    to -231, and integers greater than 231 - 1 should be clamped to 231
    - 1.
    ...
    def myAtoi(self, s: str) -> int:
        ...
        for _ in s[start:]:
            if _.isdigit():
                ...
                break
        if s[start:stop]:
            res = -int(s[start:stop]) if neg else int(s[start:stop])
        else:
            return 0
        
        min_limit = -2_147_483_648
        max_limit = 2_147_483_647

        if min_limit <= res <= max_limit:
            return res
        else:
            return min_limit if res < min_limit else max_limit 
